...
parser.add_argument("--bq-dataset", type=str, default=None, help="Dataset de BigQuery donde se escribirán las tablas")
parser.add_argument("--bq-location", type=str, default=None, help="Localización del dataset de BigQuery")
...
if not input_path or (not input_path.startswith("gs://") and not os.path.exists(input_path)):
    if input_path and not input_path.startswith("gs://") and os.path.exists("df_completo_cr_mini.csv"):
...
def load_data(src_path):
    if src_path.startswith("gs://"):
        if not gcp_active:
            raise RuntimeError("google-cloud-storage no está instalado para leer desde GCS")
...
def save_results_to_gcs(results, bucket_name, folder, file_name):
    """Guarda los resultados en un Bucket GCS como un único archivo JSON."""
    if not gcp_active:
        print("[!] google-cloud-storage no está disponible, no se puede guardar en GCS.")
        return
    ...
    print(f"[*] Resultados guardados en GCS: gs://{bucket_name}/{folder}/{file_name}")

def preprocess_and_feature_engineering(df_in):
    df_out = df_in.copy()

    # Limpieza de variables de porcentaje
    ...
    # Mapeo ordinal de antigüedad laboral
    antiguedad_map = {"< 1 year": 0.5, "1 year": 1.0, "2 years": 2.0, "3 years": 3.0, "4 years": 4.0, 
    ...
    # Mapeo ordinal de grado de riesgo
...
target_summary = [
    {
        ...
        "percentage": round(float(target_dist.get(0, 0) / total_rows_filtered) * 100, 2) if total_rows_filtered > 0 else 0,
    },
    {
        "label": "Impago/Default (Clase 1)",
        "count": int(target_dist.get(1, 0)),
        "percentage": round(float(target_dist.get(1, 0) / total_rows_filtered) * 100, 2) if total_rows_filtered > 0 else 0,
    },
]
...
if not df_num.empty:
    ...
    correlation_data["matrix"] = [
        [
            round(float(corr_matrix.loc[r, c]), 3) if not pd.isna(corr_matrix.loc[r, c]) else 0
...
ensure_bq_dataset(client=bq_client, dataset_id=bq_dataset, location=bq_location)
...
eda_results = {
    ...
    "eda_target_distribution": df_target_distribution.to_dict(orient="records"),
    "eda_descriptive_stats": df_descriptive_stats.to_dict(orient="records"),
    "eda_categorical_distribution": df_categorical_distribution.to_dict(orient="records"),
